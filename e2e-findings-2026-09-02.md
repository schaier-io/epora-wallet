...
- **"Invalid" badge on submitted proposals.** After submission the background validity check flags the request Invalid because its own inputs were spent by itself. Noise on a SUBMITTED row; submitted proposals should not run the spent-input check (or should show "Sent" instead).
- Earlier findings still open: rebuild false-success; missing wallet-name field on the deployed editor.

### PRs opened (cumulative)
- #158 witness-set fix · #159 "Multi-custodial owner" label · #160 30-min validity · #161 wallet-settings URL round-trip · #162 multisig drafts list power holders · #168 Cardanoscan links · #173 people roster with permission chips
