...
export function WalletConnectionDialog({
  ...
  const resolvedTitle = title ?? (activeWalletName ? i18n("changeConnectedWallet") : i18n("connectWallet"));
  const resolvedDescription =
    description ??
    i18n("chooseHowYouWantToOpenEpora");
          ...
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1 space-y-1">
              ...
              <p className="text-sm leading-relaxed text-muted-foreground">
                {guidedSteps
                  ? i18n("chooseTheCardanoExtensionThatWillApproveYour")
                  : i18n("connectACardanoExtensionToCreateAWallet")}
              </p>
            </div>
          </div>
            ...
            {isConnecting ? (
              <div className="mt-3 rounded-xl border border-primary/20 bg-primary/8 px-3 py-2 text-xs text-muted-foreground">
                <p className="text-foreground">
                  {i18n("approveConnectionInWalletPopup", {
                ...
                <details className="mt-2 rounded-lg border border-border/60 bg-background/45 px-3 py-2">
                  ...
                  <div className="mt-2 space-y-1.5 text-xs leading-relaxed text-muted-foreground">
                    <p>
                      {i18n("unlockTheExtensionIfNoPopupOpensSelect")}
                    </p>
                    <p>
                      {i18n("allowEporaInTheExtensionAndSwitchThe")}
                    </p>
                    <p>{i18n("thenRefreshTheListAndTryAgain")}</p>
                  </div>
                </details>
              </div>
            ) : null}
            ...
            {installedWallets.length === 0 ? (
              <div className="mt-4 flex flex-col gap-4 rounded-xl border border-dashed border-border/70 bg-background/45 p-4 sm:flex-row sm:items-start sm:gap-5 sm:p-5">
                ...
                <div className="min-w-0 space-y-2 text-center sm:text-left">
                  ...
                  <p className="text-sm leading-relaxed text-muted-foreground">
                    {i18n("installBrowserWalletThenRefresh")}
                  </p>
                </div>
              </div>
            ) : (
                ...
                {hasOnlyDemoWallet ? (
                  <div className="rounded-xl border border-dashed border-cyan-400/30 bg-cyan-500/8 p-4">
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                      {i18n("openTheDemoToExploreWithoutAnExtension")}
                    </p>
                  </div>
                ) : null}
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0 space-y-1">
                            ...
                            <p className="truncate text-xs text-muted-foreground">
                              {isDemoOption ? i18n("readOnlyPreview") : i18n("browserExtension")}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {active
                          ? isDemoOption
                            ? i18n("transactionSigningIsDisabledInDemoMode")
                            : i18n("readyToApproveActions")
                          : isDemoOption
                            ? i18n("browseTheAppWithoutAWalletExtension")
                            : i18n("connectAndApproveActions")}
                      </div>
                    </button>
                  );
                })}
