...
const formatReadable = (ms: number) => `<readable ${ms}>`;

test("one quoted row per event, with both dates and the actor", () => {
  const csv = buildActivityCsv([event()], formatUtc, formatReadable);

  const rows = csv.split("\n");
  assert.equal(rows.length, 2);
  // The second column was headed "Date (local)" while `formatWalletTransactionTime`
  // renders `defaultTimeZone`. Both columns are UTC; neither header may claim otherwise.
  assert.match(rows[0]!, /"Date \(UTC\)","Date \(readable\)","Type","Title","Amount","Actor","Fee \(lovelace\)","Tx hash"/);
  assert.doesNotMatch(rows[0]!, /local/);
  assert.match(rows[1]!, /"2025-08-24T01:46:40\.000Z"/);
  assert.match(rows[1]!, /"<readable 1756000000000>"/);
...
test("an actor without a detail is not wrapped in empty parentheses", () => {
  const csv = buildActivityCsv([event({ actorDetail: null })], formatUtc, formatReadable);
...
test("a missing block time falls back to the slot approximation or stays empty", () => {
  ...
  const csv = buildActivityCsv([dated], formatUtc, formatReadable);
  // The slot converts to a real moment on this network's timeline; the readable
  // column receives the same moment.
  ...
  const empty = buildActivityCsv([undated], formatUtc, formatReadable);
  assert.match(empty, /"",""/);
});
...
test("copy that contains quotes, commas, and newlines survives the round trip", () => {
  ...
  const csv = buildActivityCsv([hostile], formatUtc, formatReadable);
...
test("a timestamp beyond the representable Date range exports as undated", () => {
  ...
  const csv = buildActivityCsv([ancient], formatUtc, formatReadable);
  assert.match(csv, /"",""/);
});

test("an empty feed is just the header row", () => {
  const csv = buildActivityCsv([], formatUtc, formatReadable);
