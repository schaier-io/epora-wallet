...
// "stores one full recovery address and derives its sole signing key" serializes the
// edited form through a real `node --import tsx` subprocess (see the comment at its
// execFileSync call), so it stretches under full-suite worker contention and timed
// out once at vitest's 5 s default in CI, the same flake class as
// stt-spend.integration.test.tsx. In isolation it measures 1211/584/635 ms across
// three runs on a fast Apple Silicon box; the next-slowest test in this file is
// 283 ms. 15 s is about 12x the measured worst, well past the observed 5 s CI
// timeout, while still failing a genuinely hung subprocess in bounded time.
// vi.setConfig is file-scoped: vitest calls vi.resetConfig() after each file, so
// other suites keep the 5 s default.
...
describe("create flow", () => {
  ...
  it("stores one full recovery address and derives its sole signing key", { timeout: 30_000 }, () => {
    ...
    const payout = screen.getByLabelText("Payout and signing wallet");
    expect(payout).toHaveValue("");
    fireEvent.change(payout, { target: { value: address } });
    expect(payout).not.toHaveAttribute("aria-invalid", "true");
    expect(screen.queryByRole("button", { name: "Add recovery wallet" })).not.toBeInTheDocument();
    ...
    expect(readStateSections(datum).beneficiaries).toEqual([{
      alternative: 0,
      fields: [0, [paymentHash], { alternative: 1, fields: [] }, 1, {
        alternative: 0,
        fields: [
          { alternative: 0, fields: [paymentHash] },
          { alternative: 0, fields: [{ alternative: 0, fields: [
            { alternative: 0, fields: [stakeHash] }
          ] }] }
        ]
      }]
    }]);
