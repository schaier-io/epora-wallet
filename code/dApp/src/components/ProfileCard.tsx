  ...
  const tiltEngine = useMemo(() => {
    ...
    const setVarsFromXY = (x: number, y: number) => {
      ...
      const verticalRotate =
        verticalTiltMode === 'push' ? round(-Math.abs(centerY / 4)) : round(centerY / 4);
    ...
    const handleClick = () => {
      ...
      if (motionEvent && typeof motionEvent.requestPermission === 'function') {
        motionEvent
          ...
          .catch(() => {
            // user denied or unsupported — silently fall back to no-tilt
          });
      } else {
        window.addEventListener('deviceorientation', deviceOrientationHandler);
      }
      ...
      <div ref={shellRef} className="pc-card-shell">
        <section className="pc-card">
          <div className="pc-inside">
            ...
            {customContent ? (
              <div className="pc-custom-content">{customContent}</div>
            ) : (
              <>
