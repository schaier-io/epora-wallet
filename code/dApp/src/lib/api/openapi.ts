...
const TX_PATHS: Array<[string, string, string, string, z.ZodType]> = [
  ...
  [
    ...
    "Run one of the ten State transitions: `use`, `renew-proof-of-life`, `update-state`, `manage-streaming-payments`, `use-allowance`, `use-beneficiary`, `exit-beneficiary`, `payout-streaming-payment`, `cancel-streaming-payment` and `remove-access-index`. `action` selects which.",
