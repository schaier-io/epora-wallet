...
import { listProposalRecordsForParticipant as listParticipantProposalRecords } from "./list-records";
import {
...
// taking an explicit PrismaClient, so cursor decoding, the keyset
// where-composition, and visibility scoping are testable against a real
// database (list-records.test.ts). This wrapper keeps the store's
// route-facing three-argument shape.
export async function listProposalRecordsForParticipant(
  ...
  return listParticipantProposalRecords(getPrisma(), paymentKeyHash, walletUnit, options);
}
