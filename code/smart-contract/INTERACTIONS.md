...
| W5 | An earlier-beneficiary `UseBeneficiary` allows at most five native-asset rows in each wallet input or output and in each aggregate side. ADA does not count. `UseAllowance` instead limits the declared draw through the five-entry allowance bundle. Exact subtraction preserves every asset outside that draw. | `wallet/io.bounded_wallet_values_have_at_most_native_assets`, `state/allowance.expect_allowance_updates`, `wallet/rules.stt_action_allows_spend` | `UseAllowance`, operator `Use`, payout, `Consolidate`, and final-beneficiary recovery have no five-asset wallet cap. Ledger byte size and combined ExUnits still limit their input and output shapes. |
