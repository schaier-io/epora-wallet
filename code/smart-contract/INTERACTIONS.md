...
| G5 | The forwarded STT output contains only ADA and one STT. An input with legacy native assets can pass only through Admin `RunOperator(Use)`, which removes them. Other paths require non-ADA equality and therefore reject that input. Admin `Use` may add or remove ADA. Other paths may only add ADA, except payout, which preserves the full value. | `io.expect_stt_token_is_forwarded_unchanged`, `io.stt_value_preserved_with_lovelace_top_up`, `operator_handlers.eval_operator_use` | draining or junk-flooding the STT UTxO |
