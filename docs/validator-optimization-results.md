...
Measured on 2026-09-07. Baseline commit: `cd5b89e11919b0f9613d3ea4e522a476503e8432`.
After means the three source changes in this stack. The compiler is `aiken v1.1.23+8949565`.

## Native execution

Native Aiken simulations produced the following paired STT and wallet costs.
...
Evidence: `node scripts/check-entrypoint-budget.mjs` and `node scripts/check-beneficiary-distribution-native.mjs`.
...
Each row compares its layer with the preceding layer, using the named native fixture.
...
Payout matching checks the full address before tag decoding and checks asset quantity after tag equality.
...
Script bytes use `compiledCode.length / 2`, before wallet parameter application.
...
The combined scripts add 8 bytes. The payout fixture retains its 106-byte witness allowance.
...
The existing dense wallet distribution test produced these results.
...
Evidence: `beneficiary_distribution_tests.dense_divisible_input_has_no_asset_count_cap_wallet` in [budgets.json](../code/smart-contract/budgets.json).
The baseline record is the same path at the baseline commit.

## Alternatives measured

Moving the expected payout tag outside the scan alone added 3 STT bytes and 300 native payout memory units.
The retained address-first version uses fewer memory and CPU units for that fixture.

Direct `assets.reduce` produced a 9,315-byte wallet script after the payout change.
...
The `foldl2` wallet variant used 5,482,788 memory units and 1,878,792,148 CPU units for Consolidation.
Its wallet script was 9,396 bytes. The retained conditional `foldl` was cheaper in both metrics and smaller.

## Validation

Final `pnpm verify` exited with code `0`.
...
The baseline `pnpm verify` passed with Aiken `737 passed, 0 failed` and off-chain `33 passed, 0 failed`.
...
An independent final review of the three source diffs recorded `No actionable findings in the three changed files.`
That review did not run tests or establish live-chain behavior.

An intermediate off-chain run failed because the isolated workspace lacked a dependency link.
