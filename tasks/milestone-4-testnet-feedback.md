...
The Preprod launch was complete by 2026-09-14.
...
Source evidence: `origin/main` at `d3b5a2391e6f58836752ec40e39eec4139f8d886`.
The old checklist mixed completed implementation with outstanding live checks.
The subtasks now separate those states. Live checks and deployment drills retain their own evidence requirements.
...
- [x] **Public testnet deployment**. The launch and working health endpoint are recorded above and in the evidence below. The remaining deployment checks are separate tasks.
  ...
  - [x] Configure the indexer cron. `code/dApp/vercel.json:10-15` defines the five-minute schedule.
...
- [ ] **Onboarding & observability**. Implementation and live checks are separate below.
  - [x] Add faucet links, wallet installation links, and network checks. See [source locations](subtasks/m4-onboard-01-faucet-preflight.md#completed).
  ...
  - [x] Implement Sentry capture, scrubbing, and release attribution. See [source locations](subtasks/m4-onboard-03-error-sink.md#completed).
...
- [ ] **Fix loop**. The published batch is recorded. Ongoing verification remains open.
  - [x] [Intake: issue forms, categories, and Discord reports](subtasks/m4-fixloop-01-intake.md). Forms and category definitions exist. The feedback page now includes Discord issue #565.
  - [x] Publish the categorized batch and its fix links. [49 document entries, 46 categorized as fixed](subtasks/m4-fixloop-02-triage-verify.md#completed).
  - [ ] [Continue triage and verify each closure](subtasks/m4-fixloop-02-triage-verify.md#remaining-work-and-verification).
  - [x] Document the contract redeploy procedure. `docs/RUNBOOK.md:209-220`.
...
From `code/smart-contract/plutus.json` on this branch:
...
Recorded production spend on Preprod (epora.io, 4 Sep 2026, 2-of-2 send): [`64c01da1705083a6565b0f6a56cc5674396757e45426e3f80dbca3def0a6f731`](https://preprod.cardanoscan.io/transaction/64c01da1705083a6565b0f6a56cc5674396757e45426e3f80dbca3def0a6f731)

Correction: the earlier `503` sample and merge blocker are historical, not the current deployment status.
`docs/testnet-feedback.md:150` records HTTP `200`, database/indexer `up`, and `recentHeadFresh: true` on 2026-09-22.
Preprod health at 2026-09-26 00:39:26 UTC: HTTP `200`, `status: ok`, `database: up`, `indexer: up`, `recentHeadFresh: true`, `walletReconcileFresh: true`, `historyBackfillCompleted: true`.
