...
- [x] Transaction caps: a normal value-moving spend consumes at most one wallet input. The contract sets no fixed general input, output, ordinary redeemer, consolidation-input, or positive-schedule payout count. Ledger byte-size and ExUnit limits decide those transaction shapes.
- [x] Wallet Value caps: `UseAllowance`, nonfinal `UseBeneficiary`, and `Consolidate` allow at most 5 native-asset rows in each wallet input or output and in each aggregate side. ADA does not count. Operator cleanup, payout, and final-beneficiary recovery are exempt.
...
- [transaction_budget_tests.ak](../../code/smart-contract/validators/transaction_budget_tests.ak) measures grouped STT and wallet-validator costs with maximum persistent State and a minimum useful action.
