...
Correction, 2026-09-26: the earlier plan said `submit.ts` only calls `signTx` and `submitTx`. `origin/main` at `d3b5a239`, `submit.ts:20`, also calls `getNetworkId`. Any future signer interface must preserve that network check. WalletConnect signing remains deferred.
